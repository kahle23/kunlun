...
public class IdUtilTest {
    private static final Logger log = LoggerFactory.getLogger(IdUtilTest.class);
    private final Integer groupLength = FIVE;
    private final Integer count = 100;

    @Test
    public void test1() {
        IdUtil.registerGenerator("snowflake", new SnowflakeIdGenerator());
        StringBuilder builder = new StringBuilder();
        for (int i = ZERO; i < count; i++) {
            Long number = IdUtil.nextLong("snowflake");
    ...
    public void test2() {
        StringBuilder builder = new StringBuilder();
        for (int i = ZERO; i < count; i++) {
            String string = IdUtil.nextString("uuid");
