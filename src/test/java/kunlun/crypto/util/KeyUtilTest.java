...
public class KeyUtilTest {
    private static final Logger log = LoggerFactory.getLogger(KeyUtilTest.class);

    private void testGenKey(String algorithm, int keySize) {
        SecretKey secretKey = KeyUtil.genKey(algorithm, keySize);
        algorithm = secretKey.getAlgorithm();
        String format = secretKey.getFormat();
        String content = CodecUtil.encodeToHex(secretKey.getEncoded());
    ...
    private void testGenKeyPair(String algorithm, int keySize) {
        ...
        KeyPair keyPair = KeyUtil.genKeyPair(algorithm, keySize);

        PublicKey publicKey = keyPair.getPublic();
        algorithm = publicKey.getAlgorithm();
        format = publicKey.getFormat();
        content = CodecUtil.encodeToBase64(publicKey.getEncoded());
        ...
        content = CodecUtil.encodeToBase64(privateKey.getEncoded());
        log.info(privateKeyMessage, algorithm, format, keySize, content);
    }

    @Test
    public void testParseSecretKey() {
        SecretKey secretKey = KeyUtil.genKey(AES, 128);
        log.info("Secret key array: {}", Arrays.toString(secretKey.getEncoded()));
        SecretKey secretKey1 = KeyUtil.parseSecretKey(AES, secretKey.getEncoded());
    ...
    public void testParsePublicKeyAndParsePrivateKey() {
        KeyPair keyPair = KeyUtil.genKeyPair(RSA, 512);
        PrivateKey privateKey = keyPair.getPrivate();
        PublicKey publicKey = keyPair.getPublic();
        log.info("Public key array: {}", Arrays.toString(publicKey.getEncoded()));
        PublicKey publicKey1 = KeyUtil.parsePublicKey(RSA, publicKey.getEncoded());
        ...
        PrivateKey privateKey1 = KeyUtil.parsePrivateKey(RSA, privateKey.getEncoded());
    ...
    public void testToHexString() {
        SecretKey secretKey = KeyUtil.genKey(AES, 128);
        log.info("Hex string: {}", CodecUtil.encodeToHex(secretKey.getEncoded()));
        secretKey = KeyUtil.genKey(AES, 256);
        log.info("Hex string: {}", CodecUtil.encodeToHex(secretKey.getEncoded()));
    }
    ...
    public void testToBase64String() {
        SecretKey secretKey = KeyUtil.genKey(AES, 128);
        log.info("Base64 string: {}", CodecUtil.encodeToBase64(secretKey.getEncoded()));
        secretKey = KeyUtil.genKey(AES, 256);
        log.info("Base64 string: {}", CodecUtil.encodeToBase64(secretKey.getEncoded()));
    }
