 ...
 * <ul>
 *   <li><b>挂载维度：</b>挂载时由注册处指定事件类型（取值为 {@link kunlun.data.Event#getName()} 的
 *       类型词汇，如 {@link kunlun.data.Event#OPERATION_LOG} / {@link kunlun.data.Event#CHANGE_LOG} /
 *       {@link kunlun.data.Event#RUN_LOG}；{@link kunlun.data.Event#ANY} 通配任意类型）——同一类型可挂多个消费者，
 *       同一消费者也可挂到多个类型，逐一分发。</li>
 *   <li><b>批量为核心：</b>契约只有批量方法 {@link #consume(Collection)}，分发管道按事件类型
 *       分组后整批交给消费者（存储型消费者如 MySQL 获得批量写入收益）；
 *       单条便捷入口由 {@link kunlun.event.EventUtil} 基于批量方法包装，用户通常不直接调用实现类。</li>
